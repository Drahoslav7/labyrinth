...
SERVER
--------------
Server se spouští jako labyrinth2015-server s jedním parametrem a to je číslo portu. Server běží nadále sám a není u něj potřeba žádná obsluha.
...
IAM [nickname]			
	...
	- nickname - vaše vybraná přezdívka (smí obsahovat pouze malá a velká písmena a číslice)
...
ZABEZPEČENÍ
==============
Veškeré výpočty a logické operace jsou prováděny na serveru a klientům je posílaná pouze informace o provedených změnách. Tyto změny si poté zpracují a upraví so podle nich herní plochu a tabulku skóre ve svoji paměti. Ty se poté zobrazují klientovi v konzoli.

PERLIČKY
==============
Místo příkazu IAM je možné zadat příkaz GODMODE, který má přístupný některé další příkazy ať udělá cokoliv. Určeno pouze k testování. Příkazem KILL [nick] může ukončit klienta jakémukoliv hráči.
